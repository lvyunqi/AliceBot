...
当消息提到“我发的什么图”“我发的图你看明白了吗”“刚才”“上条”“这张图”等明确指代时，插件会先确定性查询当前会话的短期历史；“我发的图”会优先选取当前发言者最近发送的图片。工具调用仍可在启用 `llm.agent_enabled` 后补充查询会话历史、长期记忆和近期媒体状态，但正确识别历史图片不依赖模型是否主动调用工具。工具有 `llm.agent_max_steps` 轮上限，不执行发送、改配置或写库操作。
