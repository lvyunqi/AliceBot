...
默认情况下，模型只接收文本。为某个 provider 显式设置 `supports_vision = true` 后，AliceBot 会把当前消息中最多 4 张图片作为多模态内容块发送给该 provider；可用 `vision_model` 为图片消息单独选择模型。公开 HTTPS 图片直接发送，带签名或临时凭据的 QQ 图片会在本地通过大小和 SSRF 检查后转成 Base64 发送，凭据不会转发给模型；下载失败时不会交给文本模型猜测。未开启视觉能力的主备模型会被跳过，不会被当成识图模型。
...
当消息提到“刚才”“上条”“这张图”等指代时，启用 `llm.agent_enabled` 后模型可按需调用只读工具查询当前会话历史、当前发言者的长期记忆和最近媒体状态。工具有 `llm.agent_max_steps` 轮上限，不执行发送、改配置或写库操作。
