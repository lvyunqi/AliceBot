...
默认情况下，模型只接收文本。为某个 provider 显式设置 `supports_vision = true` 后，AliceBot 会把当前消息中最多 4 张公开 HTTPS 图片作为多模态内容块发送给该 provider；可用 `vision_model` 为图片消息单独选择模型。带签名、临时凭据或非图片媒体不会发送给模型。未开启视觉能力的主备模型只接收文本，不会被当成识图模型。
